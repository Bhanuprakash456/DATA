...
mysql_password = getenv('MYSQL_PASSWORD','bhaanu')
...
app.secret_key = b'_5#y2L"F4Q8z\n\xec]/' 
 # all the session data is encrypted in the server so we need a secret key to encrypt and decrypt the data
mysql = MySQL(app)
def update_genre(mysql):
    cur = mysql.connection.cursor()
    cur.execute("UPDATE Books SET genre = 'Entertainment' WHERE genre = 'Romance'")
...
def update_genre():
    ...
    mysql.connection.commit()
    cur.close()
    return "Genres updated from Romance to Entertainment!"
